...
def dump_zone(console, id)
  ...
  if (period.end_transition.nil? || period.end_transition.at > END_UTC)
    console.write "Fixed: #{format_offset(period.offset.utc_total_offset)} #{period.offset.abbreviation}\r\n"
  else
    while !period.end_transition.nil? && period.end_transition.at < END_UTC
      ...
      console.write "#{formatted_start} #{formatted_offset} #{period.dst? ? "daylight" : "standard"} #{period.offset.abbreviation}\r\n"
    end
  end
end
